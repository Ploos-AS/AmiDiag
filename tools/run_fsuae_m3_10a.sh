...
case "$CPU" in
  ...
  68020) MODEL=A1200; EXPECT='68010+'; BASELINE_CPU=68k ;;
...
python3 tools/capture_serial_tcp.py --port "$PORT" --output "$OUT" --connect-timeout 30 --read-timeout 15 --expect 'AMIDIAG proto=1 milestone=M3.10a cpu=68k' --expect "TEST id=CPU.BASELINE status=PASS cpu=${BASELINE_CPU} probe=family-identification"
