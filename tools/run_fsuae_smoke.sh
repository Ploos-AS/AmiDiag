...
OUT=${2:-build/m2_3-fsuae-serial.txt}
...
mkdir -p build
CFG=build/m2_3-${CHIP_KIB}.fs-uae
cat > "$CFG" <<EOF
[fs-uae]
amiga_model = A500
...
$FSUAE "$CFG" >"build/m2_3-${CHIP_KIB}-fsuae.log" 2>&1 &
EMU_PID=$!
...
"$PYTHON" tools/capture_serial_tcp.py \
  ...
  --expect 'AMIDIAG proto=1 milestone=M2.3 cpu=68000' \
...
echo "PASS: FS-UAE M2.3 data-line test and ${CHIP_KIB} KiB discovery"
