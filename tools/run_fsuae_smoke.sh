...
OUT=${2:-build/m2_7-fsuae-serial.txt}
...
case "$CHIP_KIB" in
  512) MEM_RECORD='MEM region=CHIP start=0x00000000 end=0x00080000 bytes=524288 confidence=discovered' ;;
  1024) MEM_RECORD='MEM region=CHIP start=0x00000000 end=0x00100000 bytes=1048576 confidence=discovered' ;;
  1536) MEM_RECORD='MEM region=CHIP start=0x00000000 end=0x00180000 bytes=1572864 confidence=discovered' ;;
  2048) MEM_RECORD='MEM region=CHIP start=0x00000000 end=0x00200000 bytes=2097152 confidence=discovered' ;;
  *) echo "unsupported CHIP_KIB=$CHIP_KIB" >&2; exit 2 ;;
esac

mkdir -p build
CFG=build/m2_7-${CHIP_KIB}.fs-uae
cat > "$CFG" <<EOF
[fs-uae]
amiga_model = A500
...
$FSUAE "$CFG" >"build/m2_7-${CHIP_KIB}-fsuae.log" 2>&1 &
EMU_PID=$!
...
"$PYTHON" tools/capture_serial_tcp.py \
  ...
  --expect 'AMIDIAG proto=1 milestone=M2.7 cpu=68000' \
  --expect 'BOOT phase=reset status=PASS' \
  --expect 'TEST id=BOOT.VECTORS status=PASS' \
  --expect 'TEST id=BOOT.SERIAL status=PASS' \
  --expect 'TEST id=MEM.FAULT.REPORT status=PASS fields=test,address,expected,actual format=hex32' \
...
echo "PASS: FS-UAE M2.7 RAM fault-report capability and ${CHIP_KIB} KiB discovery"
